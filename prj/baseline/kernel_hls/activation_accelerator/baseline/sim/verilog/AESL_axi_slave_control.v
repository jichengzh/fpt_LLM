...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_one_in0_data_done === 1) begin
            ...
            if (write_in0_count == in0_OPERATE_DEPTH) begin
                write_in0_run_flag <= 0; 
            ...
            if (write_in0_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (in0_c_bitwidth < 32) begin
                        in0_data_tmp_reg = mem_in0[write_in0_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < in0_c_bitwidth) begin
                                in0_data_tmp_reg[j] = mem_in0[write_in0_count][i*32 + j];
                            end
                            else begin
                    ...
                    if(image_mem_in0[write_in0_count * four_byte_num  + i]!==in0_data_tmp_reg) begin
                    write (in0_data_in_addr + write_in0_count * four_byte_num * 4 + i * 4, in0_data_tmp_reg, write_in0_resp);
                    image_mem_in0[write_in0_count * four_byte_num + i]=in0_data_tmp_reg;
                    end
                end
                process_busy = 0;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ...
        if (write_one_config_r_data_done === 1) begin
            ...
            if (write_config_r_count == config_r_OPERATE_DEPTH) begin
                write_config_r_run_flag <= 0; 
            ...
            if (write_config_r_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (config_r_c_bitwidth < 32) begin
                        config_r_data_tmp_reg = mem_config_r[write_config_r_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < config_r_c_bitwidth) begin
                                config_r_data_tmp_reg[j] = mem_config_r[write_config_r_count][i*32 + j];
                            end
                            else begin
                    ...
                    if(image_mem_config_r[write_config_r_count * four_byte_num  + i]!==config_r_data_tmp_reg) begin
                    write (config_r_data_in_addr + write_config_r_count * four_byte_num * 4 + i * 4, config_r_data_tmp_reg, write_config_r_resp);
                    image_mem_config_r[write_config_r_count * four_byte_num + i]=config_r_data_tmp_reg;
                    end
                end
                process_busy = 0;
