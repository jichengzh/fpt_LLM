...
#启动方式：python test_score_fuction.py
...
def calculate_score(config):
    ...
    for f in range(N):
        for j in range(D):
            numerator = abs(output_f32[f,j] - golden_f32[f,j])
            denominator = abs(golden_f32[f,j]) + eps
            rel_err = numerator / denominator
            base_score = base_score + calculate_point(rel_err)
        penalty = penalty + bad_count_row[f] / (D * N)

    base_score = base_score / (D * N)
    final_score = max(0.0, base_score - penalty)
    final_score_round = round(final_score, 5)

    print("函数",config,"的误差评分为：", final_score_round)
